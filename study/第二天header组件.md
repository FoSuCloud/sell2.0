...
* 子组件通过props属性接收父组件传递过来的值，然后在子组件Html元素上就可以通过绑定属性的方式使用props上面的值了,
* 需要注意的是，我们其实不一定要使用绑定的，使用绑定的目的是在接收到的属性值改变的时候，绑定的属性也可以跟着改变，某些情况下我们不需要绑定；如v-if就不需要绑定，直接使用

## 3.v-if存在意义
* 由于header.vue中的support并不一定存在值，所以需要v-if事先判定是否需要该选项，相当于非必选，不然不存在support的话就会导致错误
* v-if和v-show的区别:v-if是根据判断是否为true决定是否生成该节点,由true变为false则销毁该节点；而v-show是肯定会渲染该节点，但是为false的时候，样式为display:none;
* 由于v-if会销毁重建节点，消耗比较大，初始渲染消耗小，所以适合切换不频繁的情况；而v-show切换消耗比较小，初始渲染消耗大，所以适合切换频繁的情况
...
## 5.设置基本默认样式
* 在base.styl文件中设置body,html的默认样式，例如font-weight,font-family，这是对static/reset.css的补充

## 6.stylus图片通用样式函数的使用
* 首先在maxin.styl文件中设置一个通用的样式函数，传递url，返回对应的图片
...
---
* 由于我们使用的是transfrom:scaleY(50%)，所以缩小的是所有样式，应该是仅仅缩小border-top,因为我们其他样式都是已经缩小一倍来计算的了
...
## 10.图标与文本间隙问题(inline-block内联块元素间隙问题)
* 如果不使用font-size:0方法(导致省略号不出现)的话，那么可以把两个span元素放到同一行，然后去掉font-size:0
* 其实行内元素间隙问题是一直存在的，原因是由换行或者回车造成的，元素间的间隙出现的原因是元素标签之间的空格，把空格去掉间隙自然就会消失
* [inline-block行内元素间隙参考](https://www.jb51.net/css/645772.html)

## 11.图标消失问题
* 不设置position:absolute的话，图标没法出现，但是设置之后，没办法让左边的文本右边距出现!!
* 该问题函待解决!(好吧~我也不知道我在说啥子，貌似没问题了)
